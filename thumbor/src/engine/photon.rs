...
use crate::{
    abi::abi::{
        filter, resize, spec::Data, Contrast, Crop, Filter, Fliph, Flipv, Resize, Spec, Watermark,
    },
    engine::{Engine, SpecTransform},
};
...
lazy_static! {
    ...
    static ref WATERMARK: PhotonImage = {
        ...
        let data = include_bytes!("/Users/weixuan/code/rustcode/rustcheats/thumbor/rustacean-flat-happy.png");
...
fn image_to_buf(img: PhotonImage, format: ImageOutputFormat) -> Vec<u8> {
    ...
    let inner = Vec::with_capacity(32768);

    let mut buf = Cursor::new(inner);

    dynimage.write_to(&mut buf, format).unwrap();
